...
class LlamaDecoderLayer_TransPrune(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        attention_before_hidden_states = hidden_states.clone().detach()
        # Self Attention
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
        ...
        attention_after_hidden_states = hidden_states.clone().detach()
        hidden_states = residual + hidden_states
        ...
        ffn_before_hidden_states = hidden_states.clone().detach()
        hidden_states = self.mlp(hidden_states)
        ffn_after_hidden_states = hidden_states.clone().detach()
        ...
        if output_transf:
            attention_transf = self.compute_transf_weight(attention_before_hidden_states,
                                                        attention_after_hidden_states)
            ffn_transf = self.compute_transf_weight(ffn_before_hidden_states,
                                                    ffn_after_hidden_states)
            transf_weight = attention_transf+ffn_transf
            outputs += (transf_weight,)

        return outputs
    
    def compute_transf_weight(self,before_hidden_states,after_hidden_states):

        before_norm = torch.norm(before_hidden_states,dim=-1)
        after_norm = torch.norm(after_hidden_states,dim=-1)
        length_ratio = after_norm / before_norm

        # cosine_similarities = 1-torch.abs(F.cosine_similarity(before_hidden_states, after_hidden_states, dim=-1))
        # cosine_similarities = torch.abs(F.cosine_similarity(before_hidden_states, after_hidden_states, dim=-1))
        cosine_similarities = 1-F.cosine_similarity(before_hidden_states, after_hidden_states, dim=-1)
        exp_cosine = torch.exp(cosine_similarities)
        necc = exp_cosine / exp_cosine.sum()
        transformation_weights = length_ratio * necc

        return transformation_weights
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_TransPrune(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def transprune_forward(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if rank_layer in self.accmulating_layer_list: 
                    self.output_transf = True
                else:
                    self.output_transf = False
                layer_outputs = decoder_layer(
                        hidden_states,
                        attention_mask=attention_mask,
                        position_ids=position_ids,
                        ...
                        output_attentions=output_attentions,
                        use_cache=use_cache,
                        output_transf = self.output_transf
                )
                if self.output_transf:
                    self.transf_weight_list.append(layer_outputs[-1])
            ...
            rank_layer = layer_idx+1
            if rank_layer in self.pruning_layer_list:
                if hidden_states.shape[1]!=1:  # prefill stage or training
                    stage = self.pruning_layer_list.index(rank_layer) # determine current stage
                    (
                        position_ids,
                        attention_mask,
                        hidden_states,
                        labels   # update labels and return
                    ) = self.transprune_rank_drop(    
                        cur_num = stage,
        ...
        if hidden_states.shape[1] !=1:
            ...
            print(f"total_layers_cuda_time:{self.total_cuda_time}")
    ...
    def transprune_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            orig_hidden_states=features.clone().detach()
            self_attn = self.layers[rank_layer].self_attn

            hidden_states = self.layers[rank_layer].input_layernorm(orig_hidden_states)
            ...
            for i in range(batch_size):
                ...
                prompt_ids = image_index + image_tokens[i]
                prompt_total_len = self.prompt_len[i] + image_tokens[i]

                text_query_states = cur_query_states[:,prompt_ids:prompt_total_len,:]
                text_eager_attention_mask = cur_eager_attention_mask[:,prompt_ids:prompt_total_len,:]
                ...
                # TTV
                accmulate_transf_weight = torch.zeros(image_tokens[i]).to(attention_avg_text.device)
                for weight in self.transf_weight_list:
                    accmulate_transf_weight+=weight[i][image_index:image_index+image_tokens[i]]

                accmulate_transf_weight=accmulate_transf_weight /(accmulate_transf_weight.sum()+ 1e-8)
                attention_avg_text = attention_avg_text/(attention_avg_text.sum()+ 1e-8)

                # a=0.5
                score = accmulate_transf_weight + attention_avg_text
                # score = attention_avg_text
                top_rank_index = score.topk(keep_length[i]).indices
                top_rank_index = top_rank_index + image_index
                top_rank_index= top_rank_index.sort().values
                start_index = image_index + image_tokens[i]
                
                # Only available when batch_size is 1.
                change_weight_list = []
                for weight in self.transf_weight_list:
                    change_weight_list.append(torch.cat([weight[i, :image_index] ,
                                                    weight[i, top_rank_index], 
                                                    weight[i, start_index:]], dim=0).unsqueeze(0))
                self.transf_weight_list = change_weight_list
                #------------------------------------------------------------------------
